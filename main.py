...
Join @Tech_Muiz_Official For Updates"""
    check_if = await is_user_on_chat(bot, "@Tech_Muiz_Official", m.peer_id)
    if not check_if:
        return await m.reply("Please join @Tech_Muiz_Official then send me the link again.")
    check_if = await is_user_on_chat(bot, "@Tech_Muiz_Official", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @Tech_Muiz_Official then send me the link again."
        )
...
async def handle_message(m: Message):
    ...
    if is_spam and m.sender_id not in [5843408012]:
    ...
    if int(data["sizebytes"]) > 524288000 and m.sender_id not in [5843408012]:
        return await hm.edit(
...
Size: **{data["size"]}** 
Direct Link: [Click Here](https://t.me/teraboxtech_bot?start={uuid})
